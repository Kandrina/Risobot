...
class Satori(commands.Cog):
    ...
    async def satori(self, ctx):
        ...
        with open('json/satori.json') as f:
            data = json.load(f)
        #print(data)
        quotes = data["quotes"]
        images = data["images"]
        ...
        embedVar.add_field(name="Satori Tendou", value=quotes[random.randint(0, quotes_count)], inline=False)
        embedVar.set_thumbnail(url=images[random.randint(0, images_count)])
        await ctx.send(embed=embedVar)

def setup(bot):
    bot.add_cog(Satori(bot))
